# Renderable Codecs and Silver-Layer Publishing

This page explains how DataJoint codecs interact with downstream typed-query systems — specifically, how `<blob@>`-style opaque columns differ from typed renderable codecs, and why pipeline authors who want their data accessible to Spark SQL, BI tools, or Delta Sharing should choose typed codecs over generic blobs.

For the normative protocol contract, see [Renderable Codec Protocol Specification](../reference/specs/renderable.md). For the base `Codec` interface, see [Codec API](../reference/specs/codec-api.md).

!!! version-added "New in DataJoint 2.3"
    The `Renderable` Protocol ships in 2.3. Renderable codec implementations are out-of-tree plugins that opt in by implementing the `render_spark()` method.
...
class Recording(dj.Imported):
    ...
    -> Session
    ---
    signal : <float_array@>     # 1D float array — renderable, lands in silver as ARRAY<DOUBLE>
    metadata : <labeled_struct@>  # structured record — renderable, lands as STRUCT<...>
...
The natural-seeming design — "add a `render_spark()` method to the `Codec` base class" — was the first proposal ([#1457](https://github.com/datajoint/datajoint-python/issues/1457)). It was rejected for three reasons.

**Generic codecs can't render meaningfully.** Forcing `<blob@>` to implement `render_spark` would mean enumerating the type taxonomy of mYm-tagged binary content inside the framework — exactly the structural knowledge that `<blob@>` exists to avoid. The clean answer is: `<blob@>` does not implement the protocol. Pipeline authors who want silver migrate to a typed codec.
...
**Plugin codecs already in the wild stay valid.** Existing third-party codecs (`dj-zarr-codecs`, `dj-photon-codecs`, lab-specific packages) work unchanged through 2.3 and beyond. They opt into silver eligibility by adding a `render_spark()` method in a future release of their choosing.

The decisive factor is the **opt-in shape** of the contract. Protocol checking via `isinstance(codec, Renderable)` is structural — codec authors implement when they're ready, consumers detect support per-column at runtime. No coordination between codec releases and framework releases.
...
- **What renderable codecs exist.** Specific codec types ship downstream. The framework only provides the protocol.
...
- **What to do with non-renderable columns.** Consumers decide: skip, raise, fall back to bronze. The framework doesn't impose a policy.

This is the typical small-Protocol pattern in Python. `__iter__`, `__len__`, `__hash__` — each is one method that opts a type into a category of consumers. `Renderable` follows the same shape.

## Choosing codecs for a new pipeline

Two questions, in order:

1. **Do you need analytic access?** Will downstream consumers (Spark SQL, BI tools, Delta Sharing) query this column? If yes, choose a typed renderable codec. If no, `<blob@>` is fine and simpler.
2. **Is there a published codec for your value shape?** Check the [available codec plugins list](../how-to/use-plugin-codecs.md). If yes, use it. If no, implement one — the [Create Custom Codecs](../how-to/create-custom-codec.md) how-to covers the framework side; add a `render_spark()` method to make it silver-eligible.
...
- Spec: [Renderable Codec Protocol](../reference/specs/renderable.md) — the normative protocol.
